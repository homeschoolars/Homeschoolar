...
If AI content (roadmaps, news, assessments) isn't being generated in your webapp, it's likely because the **OpenAI API key is not configured** or is invalid.

## How to Check if OpenAI is Configured
...
```json
{
  ...
  "openai_status": "ok"       // ← Should be "ok"
}
```

If `openai_configured` is `false` or `openai_status` is `"missing"` or `"placeholder"`, the API key is not set correctly.

### 2. Check Environment Variables

The app requires the `OPENAI_API_KEY` environment variable to be set.
...
**Symptoms:**
- `openai_configured: true` but content generation fails
- Error messages about API quota or billing
- **400 Bad Request errors** in OpenAI usage dashboard
- "Failed to regenerate roadmap" errors in the webapp

**Solutions:**
1. Verify the API key is correct (no extra spaces, correct format)
2. Check OpenAI Dashboard for API quotas and billing
3. Ensure billing is enabled for your OpenAI account
...
- `openai_status: "missing"` or `"placeholder"`
...
4. Verify the key is not the placeholder value `your-openai-api-key-here`
...
4. Check OpenAI billing and quotas
